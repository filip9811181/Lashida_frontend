...
export const calculateTokenPrice = (price: bigint) => {
    ...
    return Number((1 / tokenPerUsd).toFixed(4));
};
...
export function getLeftDays(time: number) {
    return Math.floor(time / 60 / 60 / 24);
}

export function getLeftHours(time: number) {
    return Math.floor(time / 60 / 60) % 24;
}

export function getLeftMin(time: number) {
    return Math.floor(time / 60) % 60;
}

export function getLeftSecond(time: number) {
    return time % 60;
}
